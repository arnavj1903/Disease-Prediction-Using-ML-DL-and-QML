...
class FlaskMedicalAppTests(unittest.TestCase):
    ...
    def test_model_predictions(self):
        ...
        # Login using the existing test doctor from setUp
        with self.app as client:
            # Login first
            ...
            # Mock the models to avoid loading actual files
            with patch('app.models', {
                ...
                for disease in app.models:
                    num_features = len(FEATURE_LISTS[disease])
                    app.models[disease]['scaler'].transform.return_value = np.array([[0]*num_features])
                    
                    # Configure model predictions
                    app.models[disease]['DT'].predict.return_value = [1.0]
                    app.models[disease]['LR'].predict.return_value = [0.0]
                    app.models[disease]['KNN'].predict.return_value = [1.0]
                    app.models[disease]['QNN'].predict.return_value = [1.0]
                    app.models[disease]['DL'].predict.return_value = np.array([[1.0]])
                    ...
                    for label in ['positive', 'negative']:
                        ...
                        with app.app_context():
                            ...
                            self.assertIsNotNone(record, 
                                f"Patient record not created for {patient['name']} ({case['disease']})")
                            
                            # Verify prediction result matches expected
                            expected_result = 1.0 if label == 'positive' else 0.0
                            self.assertEqual(record.result, expected_result,
                                f"Prediction mismatch for {patient['name']} ({case['disease']})")
                            
                            # Verify risk label
                            expected_risk = "High Risk" if expected_result >= 0.8 else \
                                        "Medium Risk" if expected_result >= 0.5 else \
                                        "Low Risk"
                            self.assertEqual(record.risk_label, expected_risk,
                                f"Risk label mismatch for {patient['name']} ({case['disease']})")
